...
Press "va%" or "vi%" to select line(s) wrapped by tags. `M-x evilmi-select-items` does the same thing.

Press "da%" or "di%" to delete line(s) wrapped by tags. `M-x evilmi-delete-items` does the same thing.
